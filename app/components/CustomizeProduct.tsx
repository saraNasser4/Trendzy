            ...
            <ul className="flex items-center gap-4 my-4">
              {options?.choices.map((choice, i)=> {   
                ...
                return(
                  <>
                    {options.name === "Color" && 
                      <li key={choice.description} onClick={() => chooseColorHandler(choice, i)} className={`relative rounded-full w-8 h-8 ${choice.inStock ? 'cursor-pointer' : 'cursor-not-allowed'}`} style={{ backgroundColor: choice.value }}>
                    ...
                    {options.name === "Size" && 
                      <li key={choice.value} onClick={() => chooseSizeHandler(choice, i)} className={`relative rounded-xl ${choice.inStock ? 'cursor-pointer' : 'cursor-not-allowed'}`}>
