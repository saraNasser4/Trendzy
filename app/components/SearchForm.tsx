...
export default function SearchForm() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (inputSearch) router.push(`/list?name=${inputSearch}`) 
  }

  return (
    <form onSubmit={(e)=> handleSubmit(e)} className='hidden xl:block relative text-black/90'>
      <input name="input-search" className="max-w-[250px] min-w-full py-1 px-2 rounded-xl outline-none" placeholder="Search" type="text"/>
